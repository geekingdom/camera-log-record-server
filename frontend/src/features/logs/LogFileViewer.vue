...
const offset = ref(0), end = ref(0), start = ref(0), text = ref(""), busy = ref(false), eof = ref(false);
let decoder = new TextDecoder(), generation = 0;
async function read(continuing = false) {
  if (!props.file || busy.value) return;
  const current = ++generation;
  busy.value = true;
  try {
    if (!continuing) decoder = new TextDecoder();
...
<template>
  <el-dialog v-model="open" title="日志片段内容" width="min(1100px, calc(100% - 24px))" append-to-body destroy-on-close>
    ...
    <pre class="file-viewer-content" v-loading="busy">{{ text || (busy ? '' : '此范围没有内容') }}</pre>
