...
const rangeEnd = computed(() => selected.value?.end ?? 0);
  ...
  >
    ...
    <div v-if="selected" class="range-reader" aria-live="polite">
      <div class="range-reader-heading">
        <div>
          ...
          <p v-if="selectedHasLocation">仅显示该文件内的精确字节范围；跨文件的行不会被拼接。</p>
        </div>
        <el-button text @click="resetReader">返回缺口列表</el-button>
      </div>
      <el-alert
        v-if="!selectedHasLocation"
      ...
      <template v-else>
        ...
        >
          <template #default>
            <el-button :icon="RotateCcw" size="small" @click="readNext">重试读取</el-button>
          </template>
        </el-alert>
        <el-alert
    ...
    <div v-else class="range-list" role="list" aria-label="缺口范围列表">
      <article v-for="range in newestRanges" :key="range.id" class="range-item" role="listitem">
        ...
        <el-button
          v-if="range.fileId && range.start !== undefined && range.end !== undefined"
