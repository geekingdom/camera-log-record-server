...
describe("资源内存相对首个有效采样变化率", () => {
  it("每条曲线以当前查询范围内首个有效采样为基准，首点为零且保留原有缺口", () => {
    ...
    expect(result.series).toEqual([
      { key: "main", name: "主进程", unit: "%", points: [[0, null], [60_000, 0], [120_000, null], [180_000, 25]] },
      { key: "worker", name: "工作进程", unit: "%", points: [[0, 0], [60_000, -5]] },
    ]);
  ...
  it("跳过非有限值寻找首个有效基准，但零值是有效采样且不会另找非零基准", () => {
    ...
    expect(result.series[1]?.points).toEqual([[0, null], [60_000, 0], [120_000, 50]]);
  ...
  it("不根据图表缩放重算基准，调用方可仅改变纵轴窗口", () => {
    ...
    expect(result.series[0]?.points).toEqual([[0, 0], [60_000, 20], [120_000, 40]]);
  });
});
