...
def test_hourly_writer_uses_separate_paths_for_separate_tasks(tmp_path):
    async def scenario():
        one = HourlyWriter("one", "run", "session", tmp_path)
        two = HourlyWriter("two", "run", "session", tmp_path)
...
def test_archive_name_contains_safe_task_ip_shanghai_range_and_session_part(tmp_path):
    async def scenario():
        writer = HourlyWriter(
            ...
            task_name="机房/主摄像机: A", device_ip="2001:db8::10",
        )
...
def test_new_naming_skips_legacy_part_numbers_and_does_not_overwrite_same_hour(tmp_path):
    async def scenario():
        instant = datetime(2026, 9, 8, 1, 0, tzinfo=UTC)
        base = tmp_path / "task" / "run" / "session" / "2026" / "09" / "08" / "09"
        base.mkdir(parents=True)
        (base / "part-007.tar.gz").write_bytes(b"legacy")
        first = HourlyWriter("task", "run", "session", tmp_path, task_name="任务", device_ip="10.0.0.1")
        await first.write(b"one", received_at=instant)
        first_archive = await first.close()
        second = HourlyWriter("task", "run", "session", tmp_path, task_name="任务", device_ip="10.0.0.1")
