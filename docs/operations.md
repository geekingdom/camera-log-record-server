...
搜索与下载共用节点临时空间准入。每个搜索作业预留 20 GB，逐个处理快照，本地 gzip 输出和跨节点接收均在写入前检查单快照 20 GB 上限；超过限制时作业失败并清理临时文件，不返回截断快照。清理完成后释放预留，取消时仍先等待工作线程退出。节点 100 GB 准入同时计入已存在临时文件和活动预留，统计偏保守，可能提前拒绝作业。该限制不等于下载所有打包阶段的硬性总量限制，下载临时预估及最终产物空间仍需按磁盘监控管理。
