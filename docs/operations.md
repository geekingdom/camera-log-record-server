...
管理员通过控制台“后台配置”管理保留期和节点准入。保留期保存在数据库的版本化平台设置中，默认值仍来自 `RETENTION_DAYS`；首次读取会建立默认记录，此后节点维护在每轮清理前读取数据库值。修改时必须基于当前 `version`，收到 `409` 后刷新配置再决定是否重试。这个设置只影响已发布且不受下载作业保护的归档，不能用于删除 MongoDB 数据卷或正在写入的日志。

“节点登记与准入”保存管理员托管的节点 ID、服务地址、容量和准入开关。未登记 worker 为兼容原有部署仍沿用环境配置，不能将此功能视为强制节点允许列表。登记不会启动 worker、不会建立 SSH/Telnet 连接，也不会生成心跳；在线状态始终由使用同一节点 ID 的 worker 在 30 秒内上报的心跳决定。生产登记应使用不带路径、用户信息或查询参数的 HTTPS `NODE_URL`。只有本机开发可使用 `localhost` 或环回 HTTP 地址。节点报告地址与登记地址不一致时，控制台会标记 `urlMismatch`，worker 会停止接受新任务；应先核实网络与部署配置。关闭准入或调低容量不主动中断已有采集。

“服务账号”供第三方 API 调用使用。管理员创建时只显示一次明文口令，应立即存入受控密钥管理系统；服务端仅保存 SHA-256 摘要，列表和审计不会返回明文或摘要。按最小权限选择作用域，并用 `taskIds` 限制可访问设备；撤销后该账号和由其换取的下载会话都会失效。不要把服务账号令牌放入设备配置、脚本参数、Shell 历史、浏览器截图或应用日志。

“审计记录”和“运行事件”均为管理员只读视图。审计记录用于追踪控制与配置动作，运行事件用于排查连接缺口、磁盘压力、暂停和隔离等状态变化；它们只显示事件元数据和安全详情，不读取设备日志正文。查询时间需带时区，范围最多 31 天。历史 `CONNECTION_GAP` 使用 `detectedAt`，控制台会将其作为发生时间显示。
...
独立日志工作台先选择任务，再在“实时打印”“小时归档与检索”“命令记录”之间切换。小时日期筛选使用 `Asia/Shanghai` 自然日，不应把浏览器本地时区的零点直接当作归档边界。同一小时可能因重连、回拨或重新打开而有多个片段；工作台显示片段数、覆盖状态与完整性，并允许查看单个片段的 Base64 字节范围。实时缓冲报告 `gap` 时，优先在该任务的小时归档中定位片段并补读，不要把 gap 当作原始归档丢失。
...
归档发布前会验证正文与旁路 index 的大小和摘要；发布通过临时文件、`fsync` 和原子替换完成。小时目录的 `VERIFIED` 仅表示所有 READY 片段登记了正文摘要，`UNVERIFIED`、`OPEN` 和 `UNAVAILABLE` 需要结合片段详情、运行事件和下载结果处理。不要通过删除原始文件、修改目录项或手工改写摘要来“修复”完整性状态。
...
节点磁盘使用率达到 80% 时记录 WARNING，90% 时记录 NO_ADMISSION 并拒绝新建采集连接，95% 时记录 CRITICAL 并停止已有采集、将任务标记为磁盘风险错误。阈值级别变化及恢复正常均写入服务日志与 `DISK_PRESSURE_CHANGED` 事件；同一级别不按秒重复告警。

调度后的 PENDING 任务在节点建连前再次检查磁盘，防止旧心跳引起越过阈值的启动。达到 90% 时任务保留排队和端点占用，启动操作保持待完成；空间恢复至 90% 以下后自动启动。用户也可停止等待任务，节点无需建连即可结束运行、释放端点占用并完成停止操作，原待完成的启动操作标记失败。达到 95% 后已经停止的采集任务需排障后重新启动，不自动恢复。
