...
token 缓存属于每个 `PshPasswordProvider` 实例；当前采集运行会创建一个实例，不能将其视为整个 Worker 的共享缓存。token、client secret、API key、原始挑战密文和解密口令不进入诊断日志。响应只记录脱敏诊断字段或有界的非JSON错误预览，不原样保存包含凭据的完整响应。设备原始输出仍按采集合同保存，因此设备若在输出中回显挑战值或口令，该原始正文可能出现在日志文件中；部署人员须按设备输出和现有日志访问控制处理，不能把本节当作对原始采集正文的脱敏承诺。远端请求失败、超时或再次 `403003` 时，本次 `debug` 失败；设备端不会自动重发或猜测口令。

## 公司环境失败排查

优先检查任务所在Worker的运行日志，通常为`HOST_LOG_ROOT/service-logs/<节点ID>/camera-logs.jsonl`；OAuth和解密请求由Worker执行，API日志不能代替Worker日志。按`taskId`、`runId`和`sessionId`关联同一次采集会话的`PSH 解密诊断`及设备握手事件。运行日志顶层`timestamp`使用带`+08:00`的上海时间。

诊断记录区分请求开始、响应、失败及令牌刷新重试，包含接口地址（去除认证信息和查询参数）、阶段、HTTP状态、业务错误码、白名单错误说明、异常类型及耗时。非JSON返回记录内容类型、字节数和有限脱敏预览，超长内容明确截断。成功结果只记录取得有效口令，不展示口令本身。设备等待挑战、提交口令和ASH确认属于不同阶段；未取得完整挑战时不能误判为解密接口失败。

- 没有令牌请求记录：先检查是否已经ASH、是否取得完整挑战、PSH模式及配置是否启用。
- 令牌阶段失败：检查OAuth地址、client凭据、HTTP状态和错误说明；超时需检查Worker到接口的网络路径。
- 解密阶段失败：检查业务码、授权错误和响应格式；`403003`只允许一次令牌刷新后重试。
- 已取得口令但设备确认失败：对照同会话原始设备日志，排查设备拒绝、锁定或响应超时；不要连续猜测挑战码。
- 取消或总超时：同时检查任务命令的`timeoutSeconds`，它可能早于HTTP流程总预算结束。

新版本只能改善后续诊断，不能补回历史调用的响应。公司接口不可达的开发环境使用MockTransport验证失败分支，不能据此判断公司当前真实故障原因；更新Worker后复现一次并根据同会话诊断定位。
