...
归档按 Asia/Shanghai 分小时，页面明确标注北京时间 UTC+8。`2026-09-09 18:30:12 UTC` 对应北京时间 `2026-09-10 02:30:12`，所以属于次日 `02:00–03:00`；“正在写入”表示该段尚未封存。

如果真实北京时间是09日18:30，而服务器同时显示09日18:30 UTC，则服务器的绝对时钟可能快了8小时，不能只归因于页面转换。正常北京时间18:30对应UTC10:30。先运行以下只读命令检查时区与时间同步状态，并与可信时间源比较：
...
切换系统显示时区不改变绝对时间；真正的时钟偏差需按公司时间源配置NTP校时。平台不自动改主机时钟，也不把历史归档重新改名来掩盖时钟错误。
