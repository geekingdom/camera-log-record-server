...
核对日期：2026-09-09。本轮以 `345aa8e` 为基线修复审计页 Cookie 认证并收尾资源、模板、账户的变更审计事务。提交后用 `git log -1 -- docs/implementation-status.md` 定位总表版本，不把工作区改动冒充已发布。[历史记录](history/README.md) 不替代本表。

## 当前任务与恢复

当前重点：最新用户报告为“审计与事件页面误报登录失效”。已修复独立请求发送空 Bearer 覆盖 Cookie 的问题，隔离真实浏览器登录、审计筛选/详情及运行事件接口验证通过；本机旧页面刷新后需要重新登录。资源、模板、账户业务变更与审计已接入显式 Mongo 事务，真实副本集回滚、并发幂等和未知提交恢复通过，其他模块边界仍见 R26。真实 Worker 未因本轮验证重启。实时已知范围补读已完成，不重复开发；未知范围不能宣称精确恢复。前轮合成数据删除前须重新核对保护期限。白名单只约束平台客户端，不约束设备资源、采集设备及串口服务器。生成交接摘要不是新的业务目标。
...
| R18 保留天数、节点登记/准入、审计/事件 | `administration/settings.py`、节点、设置前端；审计页已复用统一 Cookie/Token 请求客户端 | 平台配置/节点测试；新增审计认证 5 项测试、隔离真实浏览器两个接口 200 和筛选详情通过 | 登记不等于部署；缺输入速率准入阈值 | 配置生效与速率准入验收；既有旧页面刷新并重新登录 |
...
| R26 平台访问记录、业务审计与凭据脱敏 | `common/audited_mutations.py`、资源/模板/账户 API：业务写入与审计同事务；资源/模板创建连同幂等成功映射提交；设备认证、哈希与 Cookie 在事务外 | 新增 helper/路由测试；`verify_audited_mutations.py` 真实副本集故障回滚、取消、12 并发幂等、重名冲突、prepare 单次执行、410 和提交确认丢失恢复通过，已接入 CI | 通用 `Repository.idem` 未迁移；登录/退出、任务/控制操作、配置、IP 规则等尚待事务审查；Cookie 签发失败不回滚已提交密码，socket 不属于数据库事务 | 继续剩余模块审计一致性；不要重复迁移已完成资源/模板/账户；保持结果未知语义，明确区分本机与 CI 部署证据 |
