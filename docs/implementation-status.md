...
核对日期：2026-09-14。本轮核对基线为 `4fadbd3`；最新业务范围为设备型号/序列号模糊筛选、资源工具栏重叠修复、趋势图构建分包，并收尾独立部署失败诊断。连接生命周期与真实协议恢复证据已提交，不重复实施。4fadbd3的CI34840657957重跑失败作业后七项成功，首次独立数据库部署失败因缺少原始输出不能确定根因。R81比率图以首个有效值为100%；R79主从SSH与R80认证退避仍保留各自未完成的实机/生产验收。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
| R86 独立部署失败输出可诊断 | `scripts/component_diagnostics.py`仅展示固定分类/提示，不回显第三方原文；`verify_component_deployment.py`接入非零退出路径 | 相关37项通过，覆盖外部Basic、含空格密码、编码值、自由文本凭据；基线CI34840657957已成功 | 首次历史故障根因未知；本机Docker未运行，新增诊断需提交后Linux CI验证；未扩展超时路径 | 查看新提交的Linux独立组件CI |
...
| R08 SSH 按凭据连接、不以变化指纹拦截，各协议保活及十秒空闲重连 | `collection/connections.py`、`collector.py`、`runtime.py`；生产行为保持；新增真实SSH及设备/串口Telnet恢复测试 | `test_ssh_runtime_reconnect.py`和`test_telnet_runtime_reconnect.py`三场景通过：服务端观察保活/命令、十秒空闲窗口、关闭后重连、预算续计及最终释放；34/35历史实机证据保留 | 本地协议不等于真实Telnet硬件协商；长期背压、集群迁移未验收；35历史13.651秒含检测与重连全程 | 继续真实硬件和多节点故障验收，不重复已经补齐的本地协议矩阵 |
