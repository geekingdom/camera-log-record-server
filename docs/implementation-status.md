...
核对日期：2026-09-11。本轮核对起点为 `35b269c`；上一轮完成数据库需求复核后，收到明确的持续总目标，继续SSH生命周期与连接名额实测。最新源码和远端同步以Git为准。`bca3cca` 及下方 CI 属于历史证据，不能证明本轮修改通过。先前阶段与实验描述已移至 [本轮前历史快照](history/2026-09-10-before-node-dashboard.md)。

## 当前状态：35设备SSH名额生命周期实测通过并完成专用日志清理
...
数据库最新要求已实现：认证历史按90天保留，连续周期成功按UTC日、身份和事务状态边界聚合，失败/人工认证/身份变化单独记录。查询和页面展示首次/最近时间及次数，区间筛选返回相交整段。每资源固定一条事务头记录保证并发状态边界；旧历史不推测合并。当前库已安装索引并回填15条旧记录的TTL。`AUTHENTICATION_RECORD_RETENTION_DAYS=0` 仅对新记录关闭TTL，已有过期时间不自动改写。审计/运行仍只有候选预览计划，没有自动删除执行器；已移除命令/操作不安全的终态清理预留配置。请求事件保持30天TTL。详情及实验见[数据库验证记录](history/2026-09-11-database-growth-validation.md)和[数据库策略](database-retention.md)。
...
| R64 数据库增长治理、认证90天保留和连续成功压缩 | `authentication_records.py`事务head/日聚合，资源API/弹窗，`event_indexes.py`，TTL回填脚本；已实现且本机安装 | 全量1113+收尾定向41；前端94/构建/1440与390；真实旧索引双初始化和33次并发认证验证；旧记录15条已回填，详见本轮验证记录 | 生产大库索引空间/写入成本未验收；runtime派生排序仍存在；审计/运行无自动删除作业 | 目标部署安装索引、低峰分批回填旧认证TTL，结合生产Explain评估后续索引 |
