...
当前重点：修复 debug 恢复后普通命令无法继续的锁存，补齐默认十秒空闲重连与真实暂停路径测试，并拆分前端认证职责。前轮 Linux 部署与账户权限已验收，不重复实施。白名单只约束平台客户端，不约束设备资源、采集设备及串口服务器。生成交接摘要不是新的业务目标。
...
| R08 SSH 按凭据连接、不以变化指纹拦截、保活及十秒空闲重连 | `collection/connections.py`、`collector.py`、`runtime.py`，已实现 | 本地 AsyncSSH 更换主机密钥测试；真实 SessionRuntime 默认十秒空闲关闭再重放初始化测试 | 本轮未重连实体设备 | 保留现有连接，CI 验证默认阈值与关闭顺序 |
