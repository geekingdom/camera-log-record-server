...
最近一次主代理全量回归868 passed（65.40秒）；之后新增的节点路由用例随消费端5项定向回归通过。前端类型检查、73项测试、生产构建、桌面/手机coredump浏览器验证通过。真实Mongo12个并发读者、12轮申请/回收竞争、委托子读者独立续租、导出工作流通过，自建库/文件已删除。此前配额事务、资源恢复竞争及2GiB双客户端下载证据见[本轮集成验证](history/2026-09-10-coredump-integration.md)，不等同于真实NFS接收验收。

核对日期：2026-09-10；本次快照读者保护基线 `caa74fd`。R32–R41 属于此前交付阶段，R42–R46 已交付源码及本地验证，目标环境验收未完成。总表只记录当前结论；此前恢复段落见[历史快照](history/2026-09-09-status-before-open-api.md)。用 `git log -1 -- docs/implementation-status.md` 定位总表版本。
...
此前R32–R41的783项后端/70项前端及六套浏览器证据见历史交付记录；不是本轮新功能验收。`caa74fd`的远端CI `34439766876`已经核实六作业全部成功，包括完整容器链路、原生部署及合成数据清理；该结果解决前一轮`34438823577`的三项验收失败，但不代表真实NFS或当前后续修改已验收。
...
快照读者保护已在独立`coredumps/snapshot_readers.py`和`snapshot_lifecycle.py`实现：同catalog行CAS协调读者与清理，RETIRING等待已有读者，DELETING保留路径/令牌直到unlink和配额收尾成功；错误中断后可重试。跨节点导出经受鉴权内部header委托子读者，父退出不影响已授权子读者续传；每块读取检查租约并保留既有限流及完整写入。真实Mongo验证已加入容器CI，本次改进的远端验收待新提交运行。仍未覆盖历史无catalog的孤立PUBLISHED声明、真实网络长时间故障与主机断电。
...
| R43 coredump多设备大文件接收、查询、批量下载；日志多人稳定下载 | `coredumps/`扫描、token冻结/续租、事务配额、异步导出；`snapshot_readers.py`读者/跨节点委托；`snapshot_lifecycle.py`可恢复清理；平台/节点Range与原生票据 | 主代理868项全量及随后消费端5项；真实Mongo12读者/12轮CAS竞争及导出通过；unlink/配额失败恢复、旧令牌隔离用例；节点ASGI委托Range验证；桌面/390浏览器通过。此前2GiB×2 HTTP+8路Range证据保留 | 真实NFS并发写入、跨机大文件导出、全部崩溃/断电窗口未验收；历史无catalog孤立声明恢复未完成；接收时间为首次扫描观测时间 | 跟进新CI，完善孤立声明恢复，再在目标Linux验证NFS及跨节点长时间混合负载 |
