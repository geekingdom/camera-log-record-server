...
核对日期：2026-09-14。本轮核对基线为 `b2103c2`，最新业务目标为R81同名进程内存对象及相对首个有效采样值的变化率图。R79主从SSH与R80认证退避仍保留各自未完成的实机/生产验收，不因新需求而丢弃。旧提交CI和服务快照不代表本轮改动。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
| --- | --- | --- | --- | --- |
| R81 同名进程统一对象与内存相对变化率 | `resource_metrics/runtime.py`按name保存process指标；`resourceMetrics.ts`历史合并及CSV；`metricRelative.ts`和趋势弹窗新增模式 | 后端1362项、前端146项；隔离browser_smoke、1440/390趋势检查通过；测试覆盖PID变化、同名累计、缺口、零基准、身份图例；Worker已加载 | 尚未用设备重启产生新PID再实测；真实长期监控验收沿用R76边界 | 核对本次提交CI，继续目标环境长期验收 |
