...
补充：设备存储目录的可读身份片段已增加跨平台安全字符清洗并限制长度；大文件并发下载继续使用流式 Range 代理，coredump 接收闭环尚待实现。
...
当前阶段：R32–R41已实现并完成本机提交前验证；本表在提交前按源码、正式接口和最终构建重新核对。阶段包括站内开放API、所有权与共享、永久服务账号及口令查看、HTTP/HTTPS实时、模板切换修复、三页批量及创建人/时间筛选。提交本阶段后暂停，等待用户手动继续。

停止边界：用户要求完成上述当前阶段需求、验证并提交代码后暂停工程需求实现，等待后续手动继续。不得在本阶段提交后自动推进后文全天压测、接管隔离或其他待办。
