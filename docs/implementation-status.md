...
核对日期：2026-09-10。已提交基线为 `e496efb`，其Linux CI七项作业全部通过，文档基线为 `ada932c`；本轮继续修复恢复任务等待认证过久的问题并更新实机证据，提交状态以Git记录为准。以下源码和测试结论只代表注明的验证范围，不代表目标生产服务器已部署。先前阶段与实验描述已移至 [本轮前历史快照](history/2026-09-10-before-node-dashboard.md)。

## 当前目标

继续核对和推进现有需求：上一阶段已交付NFS卸载、等待隔离操作、Telnet设备暂停与NFS、认证历史、core扫描、节点看板及动态分配、A/B部署入口和导航修复。本轮重点是35真实SSH生命周期验证、恢复认证响应修复、专用数据清理及总表纠偏。已完成授权的本机开发环境重建，不重复删除或重建，不保留旧无认证部署兼容分支。
...
本轮最终后端全量1056项通过（107.18秒），认证/控制/仓库/runtime定向71项通过；Ruff与diff检查通过。新增用例包含恢复请求与旧探测成功/凭据失败/离线结果交错、旧令牌不能释放新租约，以及正常资源不会因短扫描重复认证。35设备实测和数据清理范围见本轮历史记录；单次恢复2.521秒不作为集群性能保证。
...
| R08 SSH 按凭据连接、不以变化指纹拦截，各协议保活及十秒空闲重连 | `collection/connections.py`、`collector.py`、`runtime.py`；`verify_ssh_lifecycle.py`显式ID、端点分页预检及finally停止 | 既有真实AsyncSSH/TCP/Telnet及34证据保留；35本轮两轮暂停恢复加修复后一轮通过，暂停12秒socket保持0、恢复同run新session/socket1；outputClose后13.651秒恢复COLLECTING且最大连接数1，见[35实机记录](history/2026-09-10-ssh-35-lifecycle.md) | 长期背压、集群迁移未验收；13.651秒含命令、空闲检测、退避、登录与初始化，不是纯检测延迟；Telnet用例未覆盖登录认证 | 继续集群与真实网络故障验收；实机生命周期仅在空闲窗口验证 |
...
| R15 流式搜索、并发/读预算、配额与到期清理 | `logs/jobs.py`、限制器、`logs/maintenance.py`；导出清理仅允许本节点已完成到期DOWNLOAD，拒绝活跃/取消/未知记录及软链接 | 主代理维护/作业/提交/终态46项通过，包含旧mtime运行目录保护及DB单项失败继续；导出限制测试、合成报告；本机重建后当前Worker已加载 | 混合持续负载未证明；取消/无记录遗留产物保守保留 | 补持久执行归属和孤立产物恢复，并在目标环境完成维护验收 |
