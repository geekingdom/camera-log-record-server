...
核对日期：2026-09-11。本轮核对起点为 `b835bba`；继续总目标中的异常追踪和模块性，补齐空闲超时/读取失败原因事件及实测证据。最新源码和远端同步以Git为准。`bca3cca` 及下方 CI 属于历史证据，不能证明本轮修改通过。先前阶段与实验描述已移至 [本轮前历史快照](history/2026-09-10-before-node-dashboard.md)。

## 当前状态：采集重连原因事件已上线本机并完成35实测
...
| R65 SSH跨Worker每设备最多五连接、关闭确认后释放 | `collection/ssh_admission.py`、connections/runtime、Worker入口和BLOCKED恢复；已实现并加载本机 | 历史全量1130及跨进程Mongo/回环SSH五连接验收；新增35实机暂停恢复/重连/停止名额与FD验证，相关50项通过，专用日志清理完成 | 新旧Worker混跑不受完整限制，真实Linux跨机故障隔离未验收；平台外SSH不可计入控制；采样不排除间隙内重叠 | 部署先收尾旧会话并升级全部Worker，完成目标网络故障矩阵验证 |
...
以下仅为用户手动恢复后的候选事项，目前暂停执行；不得与本阶段源码交付混为一谈。恢复时先核对最新业务指令、本表、Git最新提交及工作区，再查询`711750f`的CI最终状态；重新核实服务、任务和测试产物，禁止将本表的历史时点当成当前实时状态。
