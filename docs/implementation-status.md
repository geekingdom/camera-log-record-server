...
当前推进：调度候选和领取事务改用管理员已保存的节点容量/准入配置，避免旧心跳导致关闭后继续分配或扩容暂不生效；六项新增回归、真实Mongo配置竞争与回滚、后端全量1438项及另新增批次两项、前端156项与构建通过。API已受控重载且三服务健康200，详见[保存节点准入证据](history/2026-09-15-saved-node-admission.md)。真实SSH双Worker暂停恢复已通过：A关闭连接、重新认证、B同run新session、初始化重放、定时预算累计两次、两节点各26行完整正文比对及最终清理。见[真实SSH恢复证据](history/2026-09-15-cross-worker-ssh-resume.md)；本机ISAPI使用显式隔离高端口路由，Linux CI已加入标准80端口验证步骤，结果须按本次提交核对。
