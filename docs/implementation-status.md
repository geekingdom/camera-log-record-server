...
核对日期：2026-09-11。本轮核对起点为本地提交 `8d2cacb`，新增数据库增长治理已完成验证，最新提交以Git为准，推送状态未复核。`bca3cca` 及下方 CI 属于历史证据，不能证明本轮修改通过。先前阶段与实验描述已移至 [本轮前历史快照](history/2026-09-10-before-node-dashboard.md)。

## 当前状态：数据库索引与认证历史增长治理已实现并完成本机验证
...
历史暂停前的只读核验结果：期望RUNNING的采集任务0、QUEUED/RUNNING日志作业0、35专用验收日志登记0、隔离作业恢复临时数据库0。该段不是本轮运行状态；本轮主代理与三个子代理正在推进数据库需求。

主代理负责设计、契约、审核、集成验证和总表；子代理分别实现独立模块。修改前运行CodeGraph并以源码和测试复核影响。恢复时依次对照最新业务消息、总表、git状态/最新提交、实现和证据；摘要仅是线索，生成摘要不是业务任务。

本轮新增：SSH 同一 Worker 端点五连接上限及异常回收；正式 Docker 使用公司 PyPI 源，默认 API/Worker/前端端口按最新更正为 18080/18081/5175；API 文档字段改为严格中文说明覆盖；初始化高级设置增加逐项标签和含义说明。跨多个 Worker 的设备全局五连接限制仍未实现，不能由进程内信号量替代。
