...
完成 NFS lazy 卸载、等待隔离任务操作、命令正文记录、Telnet设备暂停与NFS、资源认证历史、5秒core扫描，以及节点资源看板和最佳节点分配。同时落实完整平台A/独立Worker B原入口部署、自定义Mongo密码、默认资源页和刷新保留导航。按最新明确授权删除本机本工程旧服务和数据，按开发阶段新结构重建，不保留旧无认证部署兼容分支。
