...
核对日期：2026-09-11。本轮核对起点为 `7d5e383`。当前业务需求是数据库索引优化、认证历史仅保留约三个月及连续成功聚合；这些后端能力已实现，本轮补齐认证页面游标分页。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。

## 当前实现与验证

- 认证默认保留90天，按同资源、同UTC日和同身份的连续周期成功累加，保存首次时间、最近时间及次数。失败、人工认证和身份变化独立记录并中断成功区间；事务head保证并发边界，到期时间不随累加延长。
- 已安装认证、审计、运行和请求查询索引；本阶段实际库核验数量为8/6/5/10，认证缺失或null过期时间待回填0条。两万条合成样本中认证首屏20键/20文档、审计与请求50键/50文档；旧认证深页5020键，运行事件派生排序仍有SORT。详见[数据库复核](history/2026-09-11-database-latest-verification.md)。
...
- [CI34570026428](https://github.com/geekingdom/camera-log-record-server/actions/runs/34570026428)对应`7d5e383`，七项全部成功，包括新增真实Mongo跨进程SSH名额和真实SSH socket检查。它证明提交版本的Linux CI路径，不代表本轮未提交前端修改或物理多机、500路全天验收。
- 新集合与持续写入流程必须遵守[数据库策略](database-retention.md)：增长预算、保留期限、恢复引用、默认查询复合索引、真实Explain及分批清理边界。审计和运行记录目前没有自动删除执行器，不能将候选计划视为已生效。

## 未完成与下一步

1. 运行事件统一持久展示时间及旧数据迁移，消除派生排序后再验证游标查询；当前索引不能解决计算字段排序。
