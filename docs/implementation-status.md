...
核对日期：2026-09-11。本轮核对起点为 `2cfaab0`；最新可见业务请求是数据库索引优化、认证历史保留三个月及连续成功聚合。本轮重新验证已提交的数据库实现，不沿用交接摘要中的SSH实测任务指针，不操作设备。最新源码和远端同步以Git为准。`bca3cca` 及下方 CI 属于历史证据，不能证明本轮修改通过。先前阶段与实验描述已移至 [本轮前历史快照](history/2026-09-10-before-node-dashboard.md)。

## 当前状态：数据库索引与认证历史保留聚合已重新核实

本次定向28项通过；真实Mongo索引旧版本双初始化及并发认证聚合重新通过。当前业务库认证/审计/运行/请求集合分别有8/6/5/10个索引，认证保留配置90天，缺少过期时间的认证记录0条。审计和请求代表查询返回50条、扫描50键和50文档且无排序阶段；运行事件筛选仍有派生排序。实现与限制见R64，新增核实证据见数据库历史验证文档。SSH生命周期验证脚本的在途修改不属于本次数据库验收。
...
| --- | --- | --- | --- | --- |
| R65 SSH跨Worker每设备最多五连接、关闭确认后释放 | `collection/ssh_admission.py`、connections/runtime、Worker入口和BLOCKED恢复；已实现并加载本机 | 最终全量1130；跨进程Mongo同IP不同端口争用；真实回环SSH仅五shell、关闭后第六接入、最终连接与占位0；恢复相关46项通过 | 新旧Worker混跑不受完整限制，真实Linux跨机故障隔离未验收；平台外SSH不可计入控制 | 部署先收尾旧会话并升级全部Worker，完成目标网络故障矩阵验证 |
