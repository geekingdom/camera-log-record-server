...
核对日期：2026-09-11。本轮核对起点为 `abe6d38`。最新业务需求为资源级Coredump、CPU/内存监控默认规则、逐项失败隔离及单图切换/动态纵轴，节点资源IP准入与优先分配，以及资源任务列移到设备认证和创建用户之间；R75/R76/R77已实现并通过本机定向、浏览器与35实测。此前已完成35暂停期间真实重启、离线恢复及连接释放验收。工作台Linux归档弹窗问题的本机生产回归已通过，Linux本提交复验仍需CI。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
| R74 工作台交互纳入每次提交的生产页面回归 | `frontend/scripts/verify-workspace.mjs`及CI前端步骤新增实时终端、归档阅读器、缺口补读；补读脚本支持BASE_URL | 本机15项生产预览通过；CI34577877317其余六作业通过，前端前13脚本通过后归档阅读器失败；已增加弹窗高度余量和更严格断言 | 本机通过不替代Linux证据，专用样式实际命中及LinuxCI待复核 | 修复布局并保持原断言，重新验证LinuxCI |
