...
核对日期：2026-09-14。本轮核对基线为 `2d3308a`；R82审计来源及重复摘要已提交，按持续目标推进R83管理事件游标分页与按需统计。R81比率图以首个有效值为100%；R79主从SSH与R80认证退避仍保留各自未完成的实机/生产验收，不因新需求而丢弃。旧提交CI和服务快照不代表本轮改动。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
| --- | --- | --- | --- | --- |
| R83 管理事件游标分页和按需总数 | `administration/event_cursor.py`及query/API；前端audit分页；站内API目录；已实现 | 后端1375/前端154/构建；真实Mongo混合ID同时间10页无重漏、派生/legacy/422；第5页26键/26文档；真实Cookie翻页失败/草稿筛选/迟到统计回归，1440/390；本机API已加载 | 低选择性过滤和旧时间兼容扫描不能由游标消除；生产长期容量尚未验收 | 核对本次提交CI，按真实目标数据测低选择性过滤成本 |
