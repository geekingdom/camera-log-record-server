...
| R31 日志工作台切换不横移、资源行快捷建任务 | `styles.css`稳定滚动槽、`LogsWorkspace.vue`宽度约束、`ResourceWorkspace.vue`与App复用任务编辑器 | 浏览器1440/390页签坐标/宽度不变，折叠侧栏、两类资源模拟创建和权限隐藏通过 | 本机未复现目标浏览器原始横移；目标服务器需更新前端 | 部署后复核目标浏览器常驻滚动条设置与切换效果 |
