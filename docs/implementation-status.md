...
最近一次主代理后端全量回归918 passed（77.60秒）、Ruff全量通过，新增真实SSH保活空闲测试及实机验证脚本回归。前端未改动，沿用78项及生产构建通过证据；完整生产页面浏览器覆盖工作台、API、账号、模板、批量操作和共享Coredump开关。真实Mongo监控互斥独立复跑通过；隔离真实API/Worker/WS/小时下载通过并清理随机库与日志。34实机生命周期及此前2GiB双客户端下载证据见[本轮集成验证](history/2026-09-10-coredump-integration.md)，不能替代长时间真实设备NFS容量验收。
