...
核对日期：2026-09-14。本轮核对起点为 `056515d`。当前业务目标为R79主从SSH任务及R80认证失败退避/立即认证：主机直连用户配置端口（默认22），仅从机使用资源共享扩展端口，并在每次初始化前完成内网SSH登录；从机不参与资源级监控。认证失败10/22/46次后分别进入5分钟/1小时/每天周期，成功清零。旧提交CI和服务快照不代表本轮改动。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
