...
数据库增长策略本轮已加入：认证历史默认保留 90 天，认证列表支持基于 `createdAt,id` 的稳定游标，避免长期历史深分页 `skip`；认证查询按资源、结果、身份变更和时间建立复合索引。审计、运行、命令和操作记录默认不自动删除，需显式配置并由后续低峰维护作业分批清理；请求事件继续使用 30 天 TTL。连续成功认证聚合计数尚未接入，当前仍逐次保存认证结果，待状态转移与并发原子更新设计完成后实施。
