...
核对日期：2026-09-14。本轮核对基线为 `39d3595`，CI34843428794七项全部成功，资源身份筛选、布局修复、趋势分块及独立部署诊断已完成本阶段验收。持续目标转入连接生命周期证据核验，本轮补真实本地SSH暂停/恢复，不重写已有协议逻辑。4fadbd3的首次独立数据库部署失败仍因原始输出缺失而无法确定根因。R81比率图以首个有效值为100%；R79主从SSH与R80认证退避仍保留各自未完成的实机/生产验收。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
- 本轮按连接生命周期、时间前缀及归档命名源码审查，未发现需新增生产修复的问题。前缀/存储/故障/归档准备及SSH/Telnet重连/暂停联合28项通过；新增真实AsyncSSH暂停/恢复与相邻生命周期25项通过。暂停保持run、预算和端点锁，服务端确认shell EOF与连接关闭，admission由1变0，恢复新会话再次初始化、预算累计至2。该证据使用MongoMock，不代替真实事务或物理跨节点隔离。详见[SSH暂停恢复验收](history/2026-09-14-ssh-pause.md)。
- R84/R85/R86已实现，20项生产页面联合验证通过。前端155项、构建、Ruff通过；后端全量1394项通过后补诊断输出边界，相关37项再次通过。工具栏1920/1440/1024/390/320及桌面侧栏双状态专项检查通过；隔离真实Worker/WS/小时下载通过并删除随机库与模拟目录。本轮不连接实体设备、不产生真实设备日志。详见[资源布局及构建验收](history/2026-09-14-resource-layout-build.md)。
- 基线4fadbd3连接恢复验收使用本地真实协议服务，保留十秒默认空闲阈值，测试专用保活间隔为0.2秒。Telnet设备/串口两项及SSH单项通过：两会话真实保活、旧连接先关闭、四条初始化按服务端实收顺序重放、同运行预算累计及最终连接释放；空闲窗口单独测量为9.99到13秒之间，不能用整体运行时长替代。当时后端1380项、前端154项、构建/Ruff/diff通过；CI34840657957重跑失败作业后七项成功。详见[协议恢复证据](history/2026-09-14-protocol-reconnect.md)。SSH测试使用AsyncSSH私有保活观测钩子，升级依赖时需复核兼容性，不扩展为生产依赖锁定。R16总表已按当前源码更正：App为500行，5a0ff89已统一刷新入口，导航12项回归通过，旧重复加载待办不再成立。
...
| --- | --- | --- |
| P1 实现：节点负载准入 | 已采集inputBytesPerSecond，调度已有容量/CPU/内存/磁盘/写延迟和网络评分，但没有基于输入速率的硬阈值 | 结合实测带宽/落盘能力定义阈值并在调度及原子认领复核；证明超限不分配、恢复后可分配 |
| P1 实现：增长记录治理 | 认证90天聚合/TTL、请求30天TTL已落地；审计/运行事件只有保留计划，命令/预算/操作/run仍无自动归档删除 | 先完成恢复/幂等引用审计，再实现限量归档/清理；故障及恢复竞争验证，不能直接按终态TTL |
...
| P2 实现：实时缺口定位 | 已知file/offset可补读；缺少定位的gap需转小时归档，跨文件分别查看 | 补跨节点/跨文件未知缺口目录定位与续传验证 |
| P2 实现及验收：异常产物回收 | 已有SUCCEEDED/FAILED/EXPIRED到期本节点下载清理；取消/无记录产物仍保留，旧执行者关闭证明与回收互斥需补齐 | 精确引用/关闭证据和读者保护后受限回收；大文件中断/重启/并发下载验证 |
...
| R15 流式搜索、并发/读预算、配额与到期清理 | `logs/jobs.py`、限制器、`logs/maintenance.py:cleanup_exports`；本节点DOWNLOAD仅SUCCEEDED/FAILED/EXPIRED且有完成时间并到期才清理，拒绝活跃/取消/无记录及软链接 | `test_maintenance.py`已覆盖三种终态与拒绝条件；真实Mongo子进程崩溃明确失败证据见R26 | 混合持续负载未证明；取消/无记录产物保留，旧执行者关闭证明与回收互斥需完善 | 补有关闭/引用证据的受限回收，在目标环境完成维护验收 |
