...
当前新增业务：Linux 一键部署、用户名登录/内置管理员/子账户资源与功能权限、平台客户端 IP 多规则权限。默认管理员密码按用户明确要求为 `asdf!234`，首次强制改密；IP 规则不适用于任何设备资源或采集目标地址。上述实现本轮尚在集成验证，不能用上轮 CI 代替本轮验收。
...
| R23 Linux 一键部署、重复执行保留配置与卷 | 根 `deploy.sh`、`scripts/deploy_*`、Compose，已实现，待 Linux 验收 | 部署编排 mock 通过；CI 实际执行脚本两次；构建上下文和副本集门槛检查通过 | 当前提交真实 Linux CI 待执行 | 一键启动及重跑验收 |
| R24 正常登录、内置管理员、子账户及权限 | `users/`、`common/security.py`、前端账号模块，已实现 | 用户/资源范围测试及正式下载路由 Cookie 成功、IP 收窄和账户停用即时拒绝回归通过；模拟浏览器；开发 API 已更新 | 真实代理下改密/停用待 CI；App.vue 超过建议行数 | 隔离环境登录→改密→子账户→停用闭环 |
| R25 平台来源 IP 白名单、多网段独立权限 | `access_policy/`、Nginx、前端 IP 管理，已实现 | IPv4/IPv6、伪造头、权限交集、设备目标不受限及白名单外串口服务器 Telnet 任务创建/启动测试通过；影响图确认不触及采集模块 | 真实 Linux 代理验收待 CI | 代理链验收 |
...
本轮全量验证：后端 564 项、前端 25 项测试通过，Ruff、前端构建、构建上下文检查及 4 项副本集初始化检查通过。后续定向回归和 CI 见 [账户与部署记录](history/2026-09-09-access-and-deployment.md)。清理历史见 [清理记录](history/2026-09-09-status-and-cleanup.md)。未重跑的实体设备/集群验证不计入本轮通过项。

1. R23/R24/R25：完成当前提交 Linux CI，记录一键部署和代理下账户权限证据；R21 服务端合成数据按来源核验后清理。
