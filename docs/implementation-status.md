...
核对日期：2026-09-14。本轮核对基线为 `8b99fd4`；持续目标要求继续核验连接生命周期、协议保活、十秒无正文重连与初始化重放，本轮补真实SSH两会话及Telnet串口的恢复证据。R82重复状态事件、R83游标分页及R13归档准备修复均已提交；8b99fd4的CI34839232715七项成功。R81比率图以首个有效值为100%；R79主从SSH与R80认证退避仍保留各自未完成的实机/生产验收。恢复上下文先核对最新业务消息、此表、工作区和最新提交，摘要只作为证据线索。
...
- 本轮连接恢复验收使用本地真实协议服务，保留十秒默认空闲阈值，测试专用保活间隔为0.2秒。Telnet设备/串口两项及SSH单项通过：两会话真实保活、旧连接先关闭、四条初始化按服务端实收顺序重放、同运行预算累计及最终连接释放；空闲窗口单独测量为9.99到13秒之间，不能用整体运行时长替代。后端1380项、前端154项、构建/Ruff/diff通过；只修改测试和文档，不重启设备或变更生产配置，本轮提交CI待核对。详见[协议恢复证据](history/2026-09-14-protocol-reconnect.md)。SSH测试使用AsyncSSH私有保活观测钩子，升级依赖时需复核兼容性，不扩展为生产依赖锁定。R16总表已按当前源码更正：App为500行，5a0ff89已统一刷新入口，导航12项回归通过，旧重复加载待办不再成立。
