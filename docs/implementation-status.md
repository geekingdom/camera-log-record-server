...
本轮（2026-09-10）新增 coredump/NFS 与存储目录可读性需求：任务配置和平台 NFS 根目录字段已完成基础接入；Worker 挂载、文件接收、查询下载及部署实机验收仍未完成。

补充：型号/序列号允许缺失或为空，ISAPI 成功响应仍校验 XML 根节点和软件版本；任务身份把缺失、null、空白统一为空字符串，目录以 unknown 展示。完整目录组件最多 80 字节，截断展示部分后追加摘要，避免存储层再次截断丢失区分信息。同 IP 且两项身份均空时无法识别物理设备更换。实现位于 resources/authentication.py、tasks/resource_binding.py；验证位于 tests/test_resources.py、tests/test_task_resources.py、tests/test_storage_identity.py。目录层级简化、coredump 并发接收及大文件多人下载仍未完成，流式 Range 代码的存在不代表容量验收通过。

测试环境纠正：项目 .venv 已安装 pytest，前几轮仅使用系统 Python 的失败不能作为无法测试的依据。新增目录测试原本有错误预期，运行后已修正并完成定向回归。

本次验证：`.venv/bin/pytest -q --disable-warnings` 为 789 passed（64.54 秒），相关源码和测试 Ruff、`git diff --check` 通过；仅本机测试，未执行真实设备命令或 NFS/Linux 部署验收。

核对日期：2026-09-09。提交前基线为 `e60b85b`，本阶段实现范围为 R32–R41；本表与本阶段代码同次提交。总表只记录当前结论；此前恢复段落移至[历史快照](history/2026-09-09-status-before-open-api.md)。用 `git log -1 -- docs/implementation-status.md` 定位总表版本。
...
本轮验证：最终源码后端全量783项、前端70项通过，Ruff、类型检查与生产构建通过；真实Mongo地址占用、令牌查看/轮换审计、模板事务验证通过并清理临时库。隔离API/Worker/Cookie/WebSocket/小时下载全链路通过并清理。最终生产构建六套浏览器检查通过，覆盖1440/390/320视口、旧模板导航、权限/筛选和批量部分失败；详见交付记录。局部或mock验证不代替全天容量和目标服务器验收。当前阶段新提交的远端CI尚未产生结果，历史CI不得替代。

历史验收补齐：`e60b85b` 的 Linux CI 34346496395 已核实六作业全部成功，中文截图已收取。该结果不代替本轮新界面测试。

部署边界：本轮API最终更新至87886，目录70项（69个HTTP接口和1个WebSocket），创建用户目录、模板列表、API目录正式接口均200；1960个任务更新前后desiredState/status均STOPPED，身份/运行/会话/代次/节点摘要相同。Worker25871未重启。5173开发前端在运行。PID仅为此次验证时点；恢复须重新核实。详见[交付记录](history/2026-09-09-open-api-and-ownership.md)。
...
以下仅记录未完成项，不授权自动执行；必须等待用户手动确认继续。
