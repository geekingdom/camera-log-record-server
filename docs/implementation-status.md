...
当前重点：完成 Linux 一键部署、账户与来源 IP 权限验收，及时同步证据并清理开发产物。白名单只约束平台客户端，不约束设备资源、采集设备及串口服务器。生成交接摘要不是新的业务目标。
...
| R24 正常登录、内置管理员、子账户及权限 | `users/`、`common/security.py`、前端账号模块，已实现并通过 Linux CI | 567 项后端测试；Cookie 下载成功/权限撤销拒绝；模拟浏览器；CI 真实代理完成首登改密、子账户和停用闭环 | App.vue 超过建议行数；真实浏览器完整采集操作仍需专用模拟任务复验 | 按产品缺口推进，后续拆分认证界面装配 |
