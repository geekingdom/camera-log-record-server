...
最新推进：用户确认暂时没有目标Linux服务器，先完善本机可验证部分。文档提交`ee6b42d`的CI34917721797七项成功。新增`scripts/verify_cross_worker_gap.py`同机双Worker真实补读验证已通过：A/B/A三段各70KiB，非零锚点、4KiB续读与215004字节摘要一致；B真实停止后503而A仍200；随机库及本轮文件已回收。新增Linux CI作业结果按提交另核，该验证不代表采集连接迁移或物理网络分区。详见[双Worker补读证据](history/2026-09-15-cross-worker-gap.md)。
...
| P1 验收：多节点故障 | 单机/容器及真实Mongo已有证据，物理跨机网络分区、旧Worker隔离及接管未完成 | A完整平台+B独立Worker实际部署，断网/重启/磁盘失败/接管矩阵，证明旧连接先释放且不跨路混写 |
| P2 验收：实时缺口定位 | 跨文件/跨会话目录定位、短游标及前端顺序补读已实现；同机双真实Worker目录分页、非零锚点与字节摘要、停止单节点故障隔离验证已通过 | 新增集成CI、物理跨节点和真实持续采集；没有被服务端保存的源端正文无法补造 |
...
| R08 SSH 按凭据连接、不以变化指纹拦截，各协议保活、十秒空闲重连及暂停恢复 | `collection/connections.py`、`collector.py`、`runtime.py`、Worker.pause；生产行为保持；本地真实SSH和设备/串口Telnet测试 | 三协议重连服务端保活/命令、十秒窗口、关闭后重连及预算续计；新增`test_ssh_device_pause.py`真实EOF、暂停保留run/预算/锁、admission释放、恢复新session及四条初始化重放；相邻25项通过；34/35历史证据保留 | 本地协议不等于真实Telnet硬件协商；真实服务端主动断开、Worker失联隔离/进程退出的协议侧验证及长期背压、集群迁移未完成 | 补生产收尾入口的真实协议观测，再继续目标多节点验收；不重复已补齐的暂停/空闲矩阵 |
