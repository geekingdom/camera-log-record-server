...
最近一次主代理后端全量回归930 passed（79.11秒）、Ruff全量通过；前端85项及生产构建通过，含密码8位、Coredump稳定状态和模块拆分回归。完整生产页面浏览器覆盖工作台、API、账号、模板、批量操作和共享Coredump开关；新增Coredump自动刷新/多选保持及手机滚动状态列验证通过。真实Mongo观测到冻结及1,976,447字节临时文件验证通过并清理。既有34实机及大文件下载证据继续保留，不能替代长时间真实设备NFS容量验收。

核对日期：2026-09-10；最新远端验收基线 `9266e0f`，CI `34451201927`成功。此前共享监控、NFS直挂、Linux浏览器验证继续有效。R49/R50修复已在无活动采集/作业时加载本机API/Worker，健康200；当前本机源码包含导出清理修复。目标生产服务器仍需更新，不能把本机生效当作目标部署完成。总表只记录当前结论；此前恢复段落见[历史快照](history/2026-09-09-status-before-open-api.md)。用 `git log -1 -- docs/implementation-status.md` 定位总表版本。

## 当前任务与恢复

最新R49/R50已实现：用户密码最小8；Coredump源稳定状态与快照状态分离，单资源扫描回绕、默认10秒扫描、历史/新增标志文件隐藏，页面时间语义澄清与自动刷新。后端930项、前端85项/构建、生产浏览器及真实Mongo临时文件转换验证通过；空闲时已更新本机API/Worker，健康200，OpenAPI最小密码8。证据见[观测修复](history/2026-09-10-coredump-observation.md)。最新追加R51：设备离线后任务卡BLOCKED、上线不恢复，正在根因审查；恢复必须保留手动STOP/PAUSE意图且确认旧连接已关闭。
...
资源健康已按审核修正：后台成功不自动恢复系统停止的任务，用户编辑认证成功才授权恢复且等待旧运行收尾；手动暂停与显式resume另有等待新认证路径。后续已完成34设备正式API启动/暂停12秒/恢复/停止验证，初始化使用用户指定四条命令；没有本机NFS安装操作，NFS实机验证仅在隔离Ubuntu CI运行。
...
| R50 Coredump接收状态、时间语义、隐藏flag和子目录发现延迟 | `coredumps/scanner.py`源观测状态和单资源回绕；API过滤历史flag；CoredumpFiles自动刷新与明确时间标签 | 主代理后端930项、前端85项；真实Mongo1,976,447字节OBSERVING/CHANGING/STABLE/FROZEN；桌面/手机状态截图及选择保持通过；本机已更新 | 稳定是观测判断非设备完成通知；目标NFS部署待更新；大目录轮转延迟取决于配额与扫描耗时 | 目标部署验证；保留源文件，不按flag推断传输完成 |
| R51 设备离线后卡等待隔离，上线自动恢复原采集任务 | 根因审查中，涉及资源健康恢复资格、Worker关闭确认与运行锁释放 | 尚未完成复现/修复 | 不得仅清BLOCKED或删除未知运行锁；手动停止/暂停不自动重启 | 复现旧实例已关闭仍卡住的路径，修复并验证恢复 |
