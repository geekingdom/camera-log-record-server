...
核对日期：2026-09-09。本轮以 `c942e64` 为基线，修复手动命令队列的并发准入上限。提交后用 `git log -1 -- docs/implementation-status.md` 定位总表版本，不把工作区改动冒充已发布。[历史记录](history/README.md) 不替代本表。

## 当前任务与恢复

当前重点：完成手动命令准入与领取事务、排队守卫的验证和交付。前轮开发清理提交 `c942e64` 已通过 CI 34309802994（后端 598 项），两份合成数据仍受保护，不能提前删除。白名单只约束平台客户端，不约束设备资源、采集设备及串口服务器。生成交接摘要不是新的业务目标。
...
| R08 SSH 按凭据连接、不以变化指纹拦截、保活及十秒空闲重连 | `collection/connections.py`、`collector.py`、`runtime.py`，已实现 | 本地 AsyncSSH 更换主机密钥测试；默认十秒空闲关闭再重放初始化测试；CI 34306652015 通过 | 本轮未重连实体设备 | 受控部署后验证实体连接 |
...
| R10 手动优先、不跨会话、断线拒绝与审计 | `commands/manual_admission.py`、`manual_claim.py`、runtime/collector；原子入队与领取，排队发送前复核归属 | 真实副本集 110 请求恰好 100 准入；停止冲突、取消回滚、未知提交验证通过；实际 Collector 守卫测试 | 通用审计不在入队事务内；最终 DB 检查至 socket 写入仍需物理隔离；真实 Worker 未重启 | 受控部署后验收；审计一致性与跨节点隔离继续见 R11/R26 |
...
2. R16/R17/R12：命令草稿确认已验证，继续实时缺口与实体 debug 恢复，完成资源→采集→交互→停止→下载验收。
3. R10/R11/R26：手动准入和领取事务已完成本地验证，继续受控部署、操作审计一致性和接管隔离；不重复开发已完成预算事务。
...
R10 手动命令本轮后端全量 610 项、前端 32 项及构建、Ruff 通过；真实副本集并发准入、停止冲突、取消回滚和未知提交验证通过。新增模块分别为 39/77 行，runtime 469 行、collector 492 行。证据及未重启真实 Worker 的部署边界见 [手动命令事务记录](history/2026-09-09-manual-command-transactions.md)。
