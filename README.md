...
服务采集 SSH、Telnet 设备与 Telnet 串口日志，按会话和小时保存原始内容及归档。每个逻辑日志行在接收时均以前缀 `[YYYY-MM-DD HH:MM:SS] ` 记录服务器的 Asia/Shanghai 首字节接收时间；存储文件和实时日志使用同一份加前缀内容。SSH 和 Telnet 设备任务可暂停和恢复，海康网络资源下两种任务均可启用 Coredump NFS 监控；Telnet 串口不支持暂停、恢复和 NFS 监控。默认连续 10 秒没有收到日志时，collector 关闭当前会话并进入重连流程。SSH 使用协议 keepalive，Telnet 使用 IAC NOP 与 TCP keepalive。

控制台把实时打印、小时归档与历史检索放在独立日志工作台中，按任务选择后再建立订阅。小时目录可以按上海自然日筛选，并展示同一小时内的运行/会话片段、原始与归档大小、片段可用性和完整性汇总；实时缓冲出现缺口时可从片段字节读取接口补读。

管理员控制台还提供平台保留期、节点登记与准入、第三方服务账号、审计记录和运行事件。节点登记是持久化的人工配置，不会启动 worker 或伪造在线心跳；服务账号绑定用户并继承权限，新口令加密保存，管理员和绑定用户可随时查看，列表与审计不包含口令。详见 [docs/api.md](docs/api.md) 与 [docs/operations.md](docs/operations.md)。

服务节点看板展示 CPU、内存、磁盘、网络上下行和健康原因，新任务按容量与资源压力动态分配至适合的 Worker；运行中的采集不会为了均衡负载而迁移。指标时效、Docker 主机范围与准入规则见 [节点健康与调度](docs/node-health-scheduling.md)。

## 开发

Linux Docker 完整部署执行 `./deploy-all.sh`（兼容 `./deploy.sh`）；也提供 `deploy-frontend.sh`、`deploy-backend.sh`、`deploy-worker.sh`、`deploy-database.sh` 四个独立部署入口。Ubuntu/Debian 主机需要直接安装依赖、Nginx、MongoDB 和 systemd 服务时，使用 `sudo bash ./deploy-native.sh all --config /etc/camera-logs/native.env`，详见 [原生部署](docs/native-deployment.md)。Docker 路径的配置、日志路径、端口、保留天数、分机地址和 Docker 开机自启动见 [一键部署](docs/deployment.md) 与 `deploy/config/*.env.example`。首次管理员为 `admin`，初始密码 `asdf!234`，登录后必须修改；重复部署不会覆盖已修改密码。用户、资源范围及客户端 IP 权限见 [登录与访问控制](docs/user-access.md)。
