...
def _resolved(project: str, compose_file: Path, env_file: Path, deadline: float) -> tuple[str, str]:
...
def _worker_heartbeat(project: str, compose_file: Path, env_file: Path, node_id: str, deadline: float) -> bool:
    """在 Worker 内按其实际连接串读取一分钟心跳，兼容认证与外置 MongoDB。"""
...
async def verify():
    ...
    try:
        node = await client[settings.database_name].nodes.find_one({{"id": {node_id!r}}})
        assert node and node["heartbeat"] >= datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(seconds=60)
    finally:
        await client.close()

asyncio.run(verify())'''
    command = _compose(project, compose_file, env_file, "exec", "-T", "worker", "python", "-c", code)
...
def wait_for_health(project: str, compose_file: Path, env_file: Path, timeout: float) -> str:
    ...
    node_id, frontend_url = _resolved(project, compose_file, env_file, deadline)
    while time.monotonic() < deadline:
        ...
        if services_ok and _worker_heartbeat(project, compose_file, env_file, node_id, deadline):
            return frontend_url
