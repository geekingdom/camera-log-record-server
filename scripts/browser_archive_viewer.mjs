...
try {
  ...
  await page.locator(".archive-table .el-table__row").first().waitFor();
  assert.ok(requests.some(url => url.pathname.endsWith("/log-hours") && url.searchParams.get("date") === "2026-09-11"));
  const searchStart = page.getByLabel("检索开始时间（上海时区）", { exact: true });
  const searchEnd = page.getByLabel("检索结束时间（上海时区）", { exact: true });
  assert.equal(await searchStart.inputValue(), "2026-09-11T03:00", "默认检索开始必须为上海最近一小时");
  assert.equal(await searchEnd.inputValue(), "2026-09-11T04:00", "默认检索结束必须为上海当前分钟");
  await searchStart.fill("2026-09-11T08:15");
  await searchEnd.fill("2026-09-11T09:45");
  await date.fill("2026-09-12");
  await date.press("Enter");
  await page.waitForTimeout(100);
  assert.equal(await searchStart.inputValue(), "2026-09-11T08:15", "切换归档日期不得改写独立检索开始时间");
  assert.equal(await searchEnd.inputValue(), "2026-09-11T09:45", "切换归档日期不得改写独立检索结束时间");
  assert.ok(requests.some(url => url.pathname.endsWith("/log-hours") && url.searchParams.get("date") === "2026-09-12"), "归档目录仍按上海自然日刷新");
  ...
  assert.deepEqual(searchRequest, { taskId: task.id, keyword, start: "2026-09-11T00:15:00.000Z", end: "2026-09-11T01:45:00.000Z" });
  ...
  await page.screenshot({ path: resolve("output/playwright", "archive-search-range-1440.png"), fullPage: true, animations: "disabled" });
  await page.setViewportSize({ width: 390, height: 844 });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), true, "移动检索范围不得产生横向溢出");
  await page.screenshot({ path: resolve("output/playwright", "archive-search-range-390.png"), fullPage: true, animations: "disabled" });
